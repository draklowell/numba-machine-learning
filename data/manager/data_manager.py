...
class DataManager:
    ...
    def __init__(
        ...
        if self.process_device == Device.CPU and self.storage_device == Device.GPU:
            raise RuntimeError("Only three modes available: 'cpu to cpu', 'gpu_to_cpu', 'gpu_to_gpu'")
    ...
    def load_data(self) -> None:
        ...
        if len(self.data_cpu.shape) != 3 or self.data_cpu.shape[1:] != (28, 28) or self.data_cpu.dtype != np.uint8:
            raise ValueError(
    ...
    def get_samples(self) -> Union[np.ndarray, "cuda.devicearray.DeviceNDArray"]:
        ...
        if self.storage_device == Device.CPU:
            if self.data_cpu is None:
                raise RuntimeError("CPU data not available. Ensure downsample() has been called.")
            return self.data_cpu[indices]
        else:
            if self.data_gpu is None:
                raise RuntimeError("GPU data not available. Ensure downsample() has been called.")
