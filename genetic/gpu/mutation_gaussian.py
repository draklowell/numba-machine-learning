...
def gaussian_mutation_f64(
    ...
    if idx < tensor.shape[0]:
        if xoroshiro128p_uniform_float32(states, idx) < rate:
            noise = xoroshiro128p_normal_float64(states, idx) * strength
...
def apply_gaussian(
    ...
    blocks_per_grid = (tensor.shape[0] + (threads_per_block - 1)) // threads_per_block

    if np.issubdtype(tensor.dtype, np.float64):
        gaussian_mutation_f64[blocks_per_grid, threads_per_block, stream](
            tensor.array, states, low, high, rate, strength
        )
    else:
